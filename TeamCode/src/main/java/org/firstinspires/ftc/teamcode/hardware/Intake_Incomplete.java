...
public class Intake_Incomplete
{
    ...
    RollingAverage avgRedL = new RollingAverage(), avgGreenL = new RollingAverage(), avgBlueL = new RollingAverage(), avgDistL = new RollingAverage();
    RollingAverage avgRedM = new RollingAverage(), avgGreenM = new RollingAverage(), avgBlueM = new RollingAverage(), avgDistM = new RollingAverage();
...
//    private double _rightAvgDist, _rightAvgR, _rightAvgG, _rightAvgB;
    ...
    public void clearLeftSensorValues()
    {
        ...
        avgBlueL.reset();
        avgDistL.reset();
        _leftBallColor = BallColor.NONE;
    }
    public void clearMiddleSensorValues()
    {
        avgRedM.reset();
        avgGreenM.reset();
        avgBlueM.reset();
        avgDistM.reset();
        _middleBallColor = BallColor.NONE;
    }

    public void clearRightSensorValues()
    {
        avgRedR.reset();
        avgGreenR.reset();
        avgBlueR.reset();
...
//        _telemetry.addData("I detected the right ball", _rightBallColor);
    ...
    public BallColor detectBallSticky(
        ...
        if (sensorLocation.equals("L")){
            avgRed = avgRedL;
            avgGreen = avgGreenL;
            avgBlue = avgBlueL;
            avgDist = avgDistL;
        } else if (sensorLocation.equals("M")){
            avgRed = avgRedM;
            avgGreen = avgGreenM;
            avgBlue = avgBlueM;
            avgDist = avgDistM;
        ...
        avgRed.addReading(colorSensor.red());
        avgBlue.addReading(colorSensor.blue());
        avgGreen.addReading(colorSensor.green());
        avgDist.addReading(Double.isNaN(distance) ? 999.0 : distance);

        double avgRedValue = avgRed.getAverage();
        double avgBlueValue = avgBlue.getAverage();
        double avgGreenValue = avgGreen.getAverage();
        double avgDistValue = avgDist.getAverage();
        ...
        if(!OverrideDistanceCheck) {
            ...
            double lossThreshold = (previousColor == BallColor.GREEN || previousColor == BallColor.PURPLE)
                    ? distanceThreshold * BALL_LOSS_THRESHOLD_MULTIPLIER
                    : distanceThreshold;
